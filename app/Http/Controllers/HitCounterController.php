...
use App\Http\Requests\SaveHitCounterRequest;
...
{
    private HitCounterRepositoryInterface $hitCounterRepository;

    public function __construct(HitCounterRepositoryInterface $hitCounterRepository) 
    {
    ...
    public function store(SaveHitCounterRequest $request): JsonResponse 
    {
        $authUser = auth()->user();
        
        $hitCountDetails = $request->only([
            'article_id',
            'ip_address',
            'staying_time',
            'country',
        ...
        return response()->json(
            [
                'status' => "success",
                'data' => $this->hitCounterRepository->create($hitCountDetails)
            ],
