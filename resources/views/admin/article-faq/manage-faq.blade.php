...
@section('title', 'Manage Article FAQ')
						...
						<div class="card">
										...
										<div class="flex-grow-1 px-30 flex-grow-1 bg-img dask-bg bg-none-md" style="background-position: right bottom; background-size: auto 100%; background-image: url({{asset('ed_admin/images/svg-icon/color-svg/custom-1.svg')}})">
											<div class="row">
												<div class="col-12 col-xl-7">
													<h3 class="fw-bolder">All Article FAQ</h3>
												</div>
											</div>
										</div>
									</div>
							...
							<div class="card-body">
								...
								<div class="d-flex justify-content-end mb-3">
									<a href="{{route('admin.create.faq', ['article_id'=>$article_id])}}" class="btn btn-success btn-sm"><i class="fa-solid fa-plus"></i></i> Add New</a>
								</div>
								<table class="table table-bordered table-striped">
									...
									<tbody>
										@foreach ($allArticleFAQ as $rows)
                                            <tr>
                                                <td>{{$loop->index+1}}</td>
                                                <td>{{$rows->article->title}}</td>
                                                <td>{{$rows->faq_question}}</td>
                                                <td>{!!$rows->faq_answer!!}</td>
                                                <td>
                                                    <a href="{{route('admin.show.faq', $rows->id)}}" class="btn btn-success btn-sm">Edit</a>
												...
												<div class="modal-dialog">
													<div class="modal-content">
														...
														<div class="modal-body">
															<a href="{{route('admin.store.destroy', $rows->id)}}" class="btn btn-danger mt-2">Permanent Delete</a>
														</div>
