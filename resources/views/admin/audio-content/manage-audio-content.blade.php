...
@section('title', 'Manage audio content')
						...
						<div class="card">
										...
										<div class="flex-grow-1 px-30 flex-grow-1 bg-img dask-bg bg-none-md" style="background-position: right bottom; background-size: auto 100%; background-image: url({{asset('ed_admin/images/svg-icon/color-svg/custom-1.svg')}})">
											<div class="row">
												<div class="col-12 col-xl-7">
													<h3 class="fw-bolder">All audio content</h3>
												</div>
											</div>
										</div>
									</div>
							...
							<div class="card-body">
								...
								<table class="table table-bordered table-striped">
									...
									<tbody>
										@foreach ($allData as $leftTextvideo)
											<tr>
												<td>{{$loop->index+1}}</td>
												<td>{{$leftTextvideo->article->title}}</td>
												<td>{{$leftTextvideo->articleContent->content_subtitle}}</td>
												<td>{{$leftTextvideo->audio_title}}</td>
												<td>
													<audio controls>
														<source src="{{asset('uploads/audio-content/'.$leftTextvideo->audio)}}" type="audio/mpeg">
													</audio>
												</td>
												<td>
													<a href="{{route('admin.show.audio-content', $leftTextvideo->id)}}" class="btn btn-success btn-sm">Edit</a>
												...
												<div class="modal-dialog">
													<div class="modal-content">
														...
														<div class="modal-body">
															<a href="{{route('admin.destory.audio-content', $leftTextvideo->id)}}" class="btn btn-danger mt-2">Permanent Delete</a>
														</div>
