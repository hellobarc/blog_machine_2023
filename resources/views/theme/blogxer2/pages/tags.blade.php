                ...
                <div class="row gutters-50">
                    <div class="col-lg-8">
                        <div class="row gutters-40" id="no-equal-gallery">
                            @foreach ($findTagArticle as $tag_article)
                                <div class="col-sm-6 no-equal-item">
                                    <div class="blog-box-layout3">
                                        <div class="item-img">
                                            <a href="{{route('detail_page',['id'=>$tag_article->id,'slug'=>Str::slug($tag_article->title,'-')])}}"><img src="{{asset('uploads/article/thumbnail/'.$tag_article->thumbnail)}}" alt="blog"></a>
                                        </div>
                                        <div class="item-content">
                                            <ul class="entry-meta meta-color-dark">
                                                <li><i class="fas fa-tag"></i>{{Helper::tag_name($tag_article->tags)}}</li>
                                                <li><i class="fas fa-calendar-alt"></i>{{Helper::integerDateToString($tag_article->custom_date)}}</li>
                                                <li><i class="far fa-clock"></i>{{$tag_article->read_minutes}} Mins Read</li>
                                            </ul>
                                            <h3 class="item-title"><a href="{{route('detail_page',['id'=>$tag_article->id,'slug'=>Str::slug($tag_article->title,'-')])}}">{{$tag_article->title}}</a></h3>
                                            <p>{{$tag_article->meta_description}}</p>
                                            <div class="action-area">
                                                <a href="{{route('detail_page',['id'=>$tag_article->id,'slug'=>Str::slug($tag_article->title,'-')])}}" class="item-btn">READ MORE<i class="fas fa-arrow-right"></i></a>
                                                <ul class="response-area">
                                                    <li><a href="#"><i class="far fa-heart"></i>{{Helper::count_likes($tag_article->id)}}</a></li>
                                                    <li><a href="#"><i class="far fa-comment"></i>{{Helper::count_comments($tag_article->id)}}</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="d-flex justify-content-start">
                                @foreach ($allTags as $item)
                                    <div class="widget_tag_cloud mr-2">
                                        <a href="{{route('tag_page',['id'=>$item->id, 'slug'=>$item->name])}}">{{$item->name}}</a>
                                    </div>
                                @endforeach
                            </div>
